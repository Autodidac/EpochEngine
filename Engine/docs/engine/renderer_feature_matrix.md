...
| Framebuffers / render targets / capture | Partial | OpenGL/Vulkan/DirectX swapchain or framebuffer paths, capture bridges, and runtime surfaces exist. The shared renderer API now describes sampled render-target assets plus backend requirements for color/depth attachments, sampler use, offscreen target ownership, and presentable-surface use. Raylib is the first backend device to allocate a native sampled render texture through that spine, and OpenGL/SDL3/SFML3 now share a logical RTT graph contract plus explicit FBO/texture/sampler work order for the same `engine_arcade.screen` shape and arcade-cabinet material/model consumption. The OpenGL-family build-only gate now also proves the executed command context binds the cabinet RTT material/model, sampled texture, sampler, target, and dimensions. Native OpenGL-family FBO/texture allocation, binding, and presentation parity remain active gates. |
| Renderer resource spine | First contract slice present | `render.device` now owns formal handles/descriptors for buffers, textures, samplers, shaders, pipelines, materials, render targets, binding sets, command lists, mesh/model resources, render-pass/FrameGraph-ready targets, sampled render-texture asset plans, backend attachment/sampler/presentation requirements, default sampled-target handle allocation helpers, material texture slot descriptors, and command resource bindings. `render.graph` compiles sampled render-texture assets as one owner of color texture, sampler, and render target handles, declares material and mesh/model descriptor resources, resolves pass read/write bindings into backend handles including the sampler required for sampled render-texture/material reads, then asks the device for backend binding-set handles per pass. `render.arcade` now owns the reusable Engine Arcade screen/cabinet graph builders so the RTT target, render-surface material slot, screen mesh/model, and cabinet pass are engine-source primitives rather than duplicated self-test scaffolding. The build-only engine contract self-test compiles `engine_arcade.screen` through that spine with the null device and through `render.device_opengl_family` for OpenGL, SDL3-over-GL, and SFML-over-GL logical devices, proving the shared RTT/binding shape. It also proves the mini-arcade cabinet graph where that RTT feeds a `render_surface` material slot, screen mesh, and cabinet model pass without opening renderer contexts, checks that OpenGL-family RTT records carry the native FBO/color/depth/sampler requirements while honestly reporting native allocation as not promoted yet, and proves an injected fake native allocator can run create/begin/end/destroy for the OpenGL-family sampled-RTT path. Raylib is the first backend-native model and sampled-RTT lane: `render.device_raylib` maps model descriptors to Raylib model slots and render-texture assets to Raylib render targets while existing preview drawing remains unchanged. System Info reports declared descriptors separately from backend-native mesh/model and sampled-RTT allocation so the UI does not overclaim renderer parity. Backend-native allocation and binding behind every handle remains the next acceptance gate for OpenGL, SDL, SFML, Vulkan, and DirectX. |
| Runtime-mini render assets | Contract-backed package path started | Render-to-texture remains a core engine renderer primitive. Engine Arcade is a separate runtime-mini/game package consumer that records its default mini-runtime scene, scene inventory, render-to-texture asset role, renderer-resource requirements, and `engine_arcade.screen` 512x512 sampled target through `package.registry`; the package gate now asks for sampled render targets and binding sets explicitly. `render.arcade` provides the shared source builder for that sampled target and mini cabinet material/model graph. The engine contract harness proves that sampled target can compile as a texture/sampler/render-target/binding-set asset through the shared graph, can be consumed as a cabinet-screen material/model graph by the OpenGL-derived logical devices, and carries a backend-native requirements plan for the future FBO-backed arcade screen. Raylib can now own the native RTT handles behind that target; actual arcade-cabinet presentation and the other backend implementations remain follow-up work. |
...
- Backend-native implementations behind the formal renderer resource model:
  ...
  model-handle and sampled-RTT lane, OpenGL/SDL3/SFML3 share the first logical
  sampled-RTT plus arcade-cabinet material/model graph contract, and broader
  backend allocation/feature parity is the active gate.

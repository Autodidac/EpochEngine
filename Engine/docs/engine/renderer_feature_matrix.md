...
| Texture mapping and residency | Partial | `authoring.texture` produces validated dense RGBA8 artifacts and deterministic payload hashes through the always-built `asset.texture_artifact` schema/validator. A standalone Clang contract proves that runtime boundary with authoring and texture-editor features disabled. `render.texture.artifact` contract-maps one-time sealed RGBA8-linear mip 0 into the existing standalone residency cache using synthetic-device proof. `project.asset_registry` and `project.texture_resources` now authenticate in-memory project/source/artifact identity and own bounded T0-CPU Canvas2D bindings plus optional residency acquisition. Immutable source ownership, plan-then-commit miss eviction, transactional recreation, bounded transient replacement, forward-only backend epochs, upload accounting, and stale-handle rejection are build-proven. Serialized artifact reading, persistent imported asset IDs and rename/move migration, capability-derived admission and budgets, sRGB native consumption, project-texture binding into the live scene slot, atlas, bindless, sparse, streaming, and mip-chain residency remain incomplete. |
...
scene slot. Live allocation, drawing, project-texture binding, and Canvas2D pixel presentation

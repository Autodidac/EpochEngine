...
| Frame clear/present | Partial | `render.context_frame` now resolves one overflow-safe clipped editor viewport, top-left and bottom-left coordinates, normalized viewport, projection aspect, protected overlay/capture order, and deterministic semantic signature. OpenGL, SDL3, SFML3, Raylib3, and Software consume the shared viewport plan without changing their queue/present boundaries. SDL3 launcher, replay-backed loading, editor, resize, and clean close are accepted; DirectX/Vulkan adapter consumption plus native resize/GUI/modal/replacement pixels remain regression-sensitive. |

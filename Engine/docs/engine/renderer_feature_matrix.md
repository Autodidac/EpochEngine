...
- Linux builds require OpenGL, SDL, SFML, Raylib, Vulkan, and software context
  dependencies through the current vcpkg manifest. Runtime proof remains
  backend-specific, and WSL defaults to single-context OpenGL.
- The compatibility floor targets GTX 1660-era desktop/laptop graphics and
  equivalent APIs. Newer GPU features must be capability-gated so modern
  hardware can scale upward without raising that baseline silently.
...
| SDL3 native | Present | Present | Present | Partial | Partial | Partial |
...
| --- | --- | --- |
| Window/context bootstrap | Present | OpenGL, Raylib, SDL, SFML, Vulkan, DirectX/D3D11, software fallback, and noop/headless paths exist. Linux vcpkg builds include every non-DirectX context; runtime proof remains per backend. Normal editor use is converging toward explicit first-class context panes with inactive backends torn down rather than hidden. D3D12 is planned, not active. |

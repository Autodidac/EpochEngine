...
import core.context;             // epochnamespace::core::Context
import context.commandqueue;     // epochnamespace::core::CommandQueue

import software.state;      // s_softrendererstate, SoftRendState
import software.textures;   // Texture, TexturePtr (as in your project)
import software.renderer;   // SoftwareRenderer (as in your project)
import atlas.manager;                  // atlasmanager::atlas_vector (as in your header)
...
namespace epochnamespace::anativecontext
{
    ...
    inline void capture_frame_if_requested(
        const std::vector<std::uint32_t>& framebuffer,
        const int width,
        const int height,
        const std::uintptr_t windowId)
    {
        const auto capturePath = core::cli::reserve_capture_path("software", windowId);
        if (capturePath.empty() || framebuffer.empty() || width <= 0 || height <= 0)
            return;

        if (a_writeImageFromArgb32(capturePath, framebuffer, width, height, false))
            logger::info("Software", "Captured frame to " + capturePath.string());
        else
            logger::warn("Software", "Failed to write capture to " + capturePath.string());
    }

...
export module project.texture.resources;
...
export namespace epochengine::project_textures
{
    ...
    namespace detail
    {
        ...
        [[nodiscard]] constexpr project_assets::AssetRevision asset_revision(
            const authoring::texture::DocumentRevision& revision) noexcept
        {
            return {asset_hash(revision.content), revision.sequence};
        }

        [[nodiscard]] constexpr ResourceCode bridge_code(
            texture_artifact::BridgeCode code) noexcept
        {
            using enum texture_artifact::BridgeCode;
            switch (code)
            {
    ...
    public:
            ...
            const authoring::texture::CompiledTextureArtifact& artifact) noexcept
        {
            ...
            if (!sealed)
                return reject_publication(detail::bridge_code(sealed.code));
            ...
            if (!key)
                return reject_publication(detail::bridge_code(key.code));
            ...
            texture_artifact::ResidencySelection selection = {}) noexcept
        {
            ...
            if (!acquired)
                return reject_residency(detail::bridge_code(acquired.code));
    ...
    private:
        struct Entry final
        {
            ...
            canvas2d::LogicalTextureReference logical{};
            authoring::texture::ContentHash artifact_key{};
            authoring::texture::ContentHash payload_content{};
    ...
    namespace detail
    {
        ...
        [[nodiscard]] inline authoring::texture::CompiledTextureArtifact
            contract_artifact(

...
export namespace epochnamespace::updater
{
        ...
        const std::filesystem::path& new_binary)
    {
        ...
        bat
            << "@echo off\r\n"
            << "setlocal\r\n"
            << "set TARGET=" << system_detail::quote_shell_arg(target_binary.string()) << "\r\n"
            << "set NEWBIN=" << system_detail::quote_shell_arg(new_binary.string()) << "\r\n"
            << "for /L %%I in (1,1,20) do (\r\n"
            << "  del /F /Q %TARGET% >nul 2>&1\r\n"
            << "  move /Y %NEWBIN% %TARGET% >nul 2>&1\r\n"
            << "  if exist %TARGET% goto replaced\r\n"
            << "  timeout /t 1 >nul\r\n"
            << ")\r\n"
            << "exit /b 1\r\n"
            << ":replaced\r\n"
        ...
        const std::filesystem::path& package_archive)
    {
        ...
        bat
            << "@echo off\r\n"
            << "setlocal\r\n"
            << "set EXTRACTED=" << system_detail::quote_shell_arg(extracted_runtime_dir.string()) << "\r\n"
            << "set TARGETDIR=" << system_detail::quote_shell_arg(target_dir.string()) << "\r\n"
            << "set TARGETEXE=" << system_detail::quote_shell_arg(target_binary.string()) << "\r\n"
            << "set ARCHIVE=" << system_detail::quote_shell_arg(package_archive.string()) << "\r\n"
            << "for /L %%I in (1,1,20) do (\r\n"
            << "  robocopy %EXTRACTED% %TARGETDIR% /E /NFL /NDL /NJH /NJS /NC /NS >nul\r\n"
            << "  if errorlevel 8 (\r\n"
            << "    timeout /t 1 >nul\r\n"
            << "  ) else (\r\n"
            << "    goto replaced\r\n"
        ...
        const std::filesystem::path& package_archive)
    {
        ...
        bat
            << "@echo off\r\n"
            << "setlocal\r\n"
            << "set BUILTDIR=" << system_detail::quote_shell_arg(built_runtime_dir.string()) << "\r\n"
            << "set TARGETDIR=" << system_detail::quote_shell_arg(target_dir.string()) << "\r\n"
            << "set TARGETEXE=" << system_detail::quote_shell_arg(target_binary.string()) << "\r\n"
            << "set EXENAME=" << system_detail::quote_shell_arg(target_binary.filename().string()) << "\r\n"
            << "set SRCASSETS=" << system_detail::quote_shell_arg(source_assets_dir.string()) << "\r\n"
            << "set DSTASSETS=" << system_detail::quote_shell_arg(target_assets_dir.string()) << "\r\n"
            << "set SRCROOT=" << system_detail::quote_shell_arg(source_root.string()) << "\r\n"
            << "set ARCHIVE=" << system_detail::quote_shell_arg(package_archive.string()) << "\r\n"
            << "for /L %%I in (1,1,20) do (\r\n"
            << "  robocopy %BUILTDIR% %TARGETDIR% %EXENAME% *.dll *.manifest /NFL /NDL /NJH /NJS /NC /NS >nul\r\n"
            << "  if errorlevel 8 (\r\n"
            << "    timeout /t 1 >nul\r\n"
            << "  ) else (\r\n"
            << "    goto copied\r\n"

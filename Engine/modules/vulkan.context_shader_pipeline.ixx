...
namespace epochengine::vulkancontext
{
    ...
    {
        [[nodiscard]] bool looks_like_spirv(const std::vector<char>& b) noexcept
        {
            if (b.size() < 4) return false;
            const auto u0 = static_cast<std::uint8_t>(b[0]);
            const auto u1 = static_cast<std::uint8_t>(b[1]);
            const auto u2 = static_cast<std::uint8_t>(b[2]);
            const auto u3 = static_cast<std::uint8_t>(b[3]);
            const std::uint32_t magic = (std::uint32_t)u0 |
                ((std::uint32_t)u1 << 8) | ((std::uint32_t)u2 << 16) | ((std::uint32_t)u3 << 24);
            return magic == 0x07230203u;
        }

        [[nodiscard]] std::string first_bytes_hex(const std::vector<char>& b, std::size_t n = 16)
        {
            static constexpr char hex[] = "0123456789ABCDEF";
            std::string out;
            const std::size_t m = (std::min)(n, b.size());
            out.reserve(m * 3);
            for (std::size_t i = 0; i < m; ++i)
            {
                const auto u = static_cast<std::uint8_t>(b[i]);
    ...
    inline std::vector<char> Application::readFile(const std::string& filename)
    {
        ...
        for (const auto& path : candidates)
        {
            ...
            // Replace this with epochengine::core::log::write(os.str()) if you have it.

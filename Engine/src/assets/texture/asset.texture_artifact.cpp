...
namespace epochengine::asset::texture
{
    ...
    SerializedTextureArtifact serialize_compiled_artifact(
        const CompiledTextureArtifact& artifact) noexcept
    {
        if (!validate_compiled_artifact(artifact))
            return {ArtifactIoStatus::invalid_artifact, {}};

        try
        {
            constexpr std::array<std::byte, 8> magic{
                std::byte{'E'}, std::byte{'P'}, std::byte{'T'}, std::byte{'E'},
                std::byte{'X'}, std::byte{'A'}, std::byte{'0'}, std::byte{'1'}};

            std::vector<std::byte> bytes{};
            bytes.reserve(static_cast<std::size_t>(
                160ull + artifact.identity.estimated_artifact_bytes));
            bytes.insert(bytes.end(), magic.begin(), magic.end());

            const auto append_integer = [&bytes]<typename Value>(
            ...
            const auto append_hash = [&append_integer](
            ...
            append_integer(std::uint32_t{1});
            append_hash(artifact.identity.key);
            append_hash(artifact.identity.source_revision.content);
            append_integer(artifact.identity.source_revision.sequence);
            append_integer(artifact.identity.profile.format);
            append_integer(artifact.identity.profile.color_space);
            append_integer(artifact.identity.profile.mipmaps);
            append_integer(artifact.identity.profile.compiler_schema_version);
            append_integer(artifact.identity.profile.quality_tier);
            append_integer(artifact.identity.width);
            append_integer(artifact.identity.height);
            append_integer(artifact.identity.mip_count);
            append_integer(artifact.identity.estimated_artifact_bytes);
            append_hash(artifact.payload_content);

            for (const CompiledTextureMip& mip : artifact.mips)
            {
                append_integer(mip.width);
                append_integer(mip.height);
                append_integer(mip.row_pitch_bytes);
                append_integer(static_cast<std::uint64_t>(mip.texels.size()));
                append_hash(mip.content);
                bytes.insert(bytes.end(), mip.texels.begin(), mip.texels.end());
            }

            return {ArtifactIoStatus::ready, std::move(bytes)};
        }
        catch (const std::bad_alloc&)
        {
            return {ArtifactIoStatus::allocation_failed, {}};
        }
        catch (...)
        {
            return {ArtifactIoStatus::invalid_artifact, {}};
    ...
    DeserializedTextureArtifact deserialize_compiled_artifact(
        std::span<const std::byte> bytes,
        ArtifactReadLimits limits) noexcept
    {
        if (bytes.empty())
            return {ArtifactIoStatus::empty_input, {}};
        if (limits.maximum_serialized_bytes == 0
        ...
        {
            return {ArtifactIoStatus::budget_exceeded, {}};
        }

        try
        {
            std::size_t cursor{};
            const auto read_integer = [&bytes, &cursor]<typename Value>(
                Value& value) noexcept
            {
                using Raw = std::conditional_t<
                    std::is_enum_v<Value>,
                ...
                if (cursor > bytes.size()
                    || sizeof(Unsigned) > bytes.size() - cursor)
                {
                    return false;
                }
                Unsigned bits{};
            ...
            const auto read_hash = [&read_integer](
                ContentHash& hash) noexcept
            {
                for (std::uint64_t& word : hash.words)
                {
                    if (!read_integer(word))
                        return false;
                }
                return true;
            };
            ...
            if (bytes.size() < magic.size())
                return {ArtifactIoStatus::truncated, {}};
            if (!std::equal(magic.begin(), magic.end(), bytes.begin()))
                return {ArtifactIoStatus::invalid_magic, {}};
            cursor = magic.size();

            std::uint32_t schema{};
            if (!read_integer(schema))
                return {ArtifactIoStatus::truncated, {}};
            if (schema != 1)
                return {ArtifactIoStatus::unsupported_schema, {}};

            CompiledTextureArtifact artifact{};
            auto& identity = artifact.identity;
            if (!read_hash(identity.key)
                || !read_hash(identity.source_revision.content)
                || !read_integer(identity.source_revision.sequence)
                || !read_integer(identity.profile.format)
                || !read_integer(identity.profile.color_space)
                || !read_integer(identity.profile.mipmaps)
                || !read_integer(identity.profile.compiler_schema_version)
                || !read_integer(identity.profile.quality_tier)
                || !read_integer(identity.width)
                || !read_integer(identity.height)
                || !read_integer(identity.mip_count)
                || !read_integer(identity.estimated_artifact_bytes)
                || !read_hash(artifact.payload_content))
            {
                return {ArtifactIoStatus::truncated, {}};
            }

            if (identity.profile.format > ArtifactFormat::astc_4x4_rgba
                || identity.profile.color_space > ColorSpace::srgb
                || identity.profile.mipmaps
                    > MipmapPolicy::generate_normal_renormalized)
            {
                return {ArtifactIoStatus::invalid_enum, {}};
            }
            if (identity.width == 0
                || identity.height == 0
                || identity.width > limits.maximum_dimension
                || identity.height > limits.maximum_dimension
                || identity.mip_count == 0
                || identity.mip_count > limits.maximum_mip_count
                || identity.estimated_artifact_bytes == 0
                || identity.estimated_artifact_bytes
            ...
            artifact.mips.reserve(identity.mip_count);
            std::uint64_t payloadBytes{};
            for (std::uint8_t index = 0;
                 index < identity.mip_count;
                 ++index)
            {
                CompiledTextureMip mip{};
                std::uint64_t mipBytes{};
                if (!read_integer(mip.width)
                    || !read_integer(mip.height)
                    || !read_integer(mip.row_pitch_bytes)
                    || !read_integer(mipBytes)
                    || !read_hash(mip.content))
                {
                    return {ArtifactIoStatus::truncated, {}};
                }
                if (mipBytes > limits.maximum_payload_bytes - payloadBytes)
                    return {ArtifactIoStatus::budget_exceeded, {}};
                if (cursor > bytes.size() || mipBytes > bytes.size() - cursor)
                    return {ArtifactIoStatus::truncated, {}};
                ...
                mip.texels.assign(
                    begin,
                    begin + static_cast<std::ptrdiff_t>(mipBytes));
                cursor += static_cast<std::size_t>(mipBytes);
                payloadBytes += mipBytes;
                artifact.mips.push_back(std::move(mip));
            }

            if (cursor != bytes.size()
                || payloadBytes != identity.estimated_artifact_bytes)
            {
                return {ArtifactIoStatus::invalid_artifact, {}};
            }
            ...
            if (!validate_compiled_artifact(artifact))
                return {ArtifactIoStatus::invalid_artifact, {}};
            return {ArtifactIoStatus::ready, std::move(artifact)};
        }
        catch (const std::bad_alloc&)
        {
            return {ArtifactIoStatus::allocation_failed, {}};
    ...
    const char* artifact_io_status_name(ArtifactIoStatus status) noexcept
    {
        switch (status)
        {
        case ArtifactIoStatus::ready: return "ready";
        case ArtifactIoStatus::invalid_artifact: return "invalid_artifact";
        case ArtifactIoStatus::empty_input: return "empty_input";
        case ArtifactIoStatus::invalid_magic: return "invalid_magic";
        case ArtifactIoStatus::unsupported_schema: return "unsupported_schema";
        case ArtifactIoStatus::truncated: return "truncated";
        case ArtifactIoStatus::invalid_enum: return "invalid_enum";
        case ArtifactIoStatus::budget_exceeded: return "budget_exceeded";
        case ArtifactIoStatus::allocation_failed: return "allocation_failed";
        }
        return "unknown";
    }

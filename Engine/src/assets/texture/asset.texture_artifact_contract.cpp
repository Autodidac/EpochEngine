...
namespace epochengine::asset::texture::contract
{
    ...
    [[nodiscard]] int run_runtime_artifact_contract()
    {
        ...
        const SerializedTextureArtifact serialized =
            serialize_compiled_artifact(valid);
        if (!serialized || serialized.bytes.size() <= valid.identity.estimated_artifact_bytes)
            return 11;

        const DeserializedTextureArtifact roundTrip =
            deserialize_compiled_artifact(serialized.bytes);
        if (!roundTrip
            || roundTrip.artifact.identity.key != valid.identity.key
            || roundTrip.artifact.payload_content != valid.payload_content
            || roundTrip.artifact.mips.front().texels
                != valid.mips.front().texels)
        {
            return 12;
        }

        std::vector<std::byte> malformed = serialized.bytes;
        malformed.pop_back();
        if (deserialize_compiled_artifact(malformed).status
            != ArtifactIoStatus::truncated)
        {
            return 13;
        }

        malformed = serialized.bytes;
        malformed.front() ^= std::byte{0x01};
        if (deserialize_compiled_artifact(malformed).status
            != ArtifactIoStatus::invalid_magic)
        {
            return 14;
        }

        ArtifactReadLimits constrained{};
        constrained.maximum_payload_bytes =
            valid.identity.estimated_artifact_bytes - 1;
        if (deserialize_compiled_artifact(
                serialized.bytes,
                constrained).status != ArtifactIoStatus::budget_exceeded)
        {
            return 15;
        }
        return 0;
...
int main()
{
    return epochengine::asset::texture::contract::

...
namespace epochengine
{
    ...
    EditorFrameResult editor_run(const std::shared_ptr<core::Context>& ctx)
    {
        ...
        auto launch_active_project_context = [&]()
        {
            ...
            const project_lifecycle::Evidence lifecycleEvidence{
                .project_selected = !editor.projectRoot.empty(),
                .scene_committed = true,
                .shell_materialized =
                    path_exists(projectRoot / "build_project.ps1")
                    || path_exists(projectRoot / "build_project.sh"),
                .build_inputs_current = !freshness.rebuildRequired,
                .output_exists = path_exists(outputExe),
                .build_active = false,
                .runtime_active = false};
            const project_lifecycle::Decision lifecycleDecision =
                project_lifecycle::decide(
                    project_lifecycle::Request::run,
                    lifecycleEvidence);

            if (lifecycleDecision.action
                == project_lifecycle::Action::build_project
                || lifecycleDecision.action
                    == project_lifecycle::Action::materialize_shell)
            {
                start_project_build(
                    editor,
                    true,
                    std::string("Launch Single Context - ")
                        + freshness.reason);
                return;
            }
            if (lifecycleDecision.action
            ...
            {
                editor.projectBuildStatus =
                    "Launch blocked because project lifecycle evidence is incomplete.";
                push_editor_log(
                    editor,
                    "[project] Launch blocked: lifecycle evidence did not authorize a child runtime.");
                return;
            }

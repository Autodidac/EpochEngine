...
namespace epochnamespace::core
{
    ...
    [[nodiscard]] inline int run_editor_project_self_test(std::string_view project_id)
    {
        ...
        packet.operator_notes =
            "Generated by --editor-project-self-test so EpochBot can learn from the real editor/tool loop instead of a silent build.";
        packet.control_loop_stage = build.succeeded
            ? "Verifier staged: materialize and build evidence available"
            : "Builder blocked: inspect generated project build log";
        packet.review_gate_state = build.succeeded ? "ready_for_human_review" : "blocked_build_failed";

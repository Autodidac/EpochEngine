...
namespace runtime
{
    namespace
    {
        ...
        int run_legacy_runtime(bool editor_requested)
        {
            runtime_info(
                editor_requested
                    ? "epoch runtime -> legacy editor"
                    : "epoch runtime -> legacy engine");

            return epochengine::core::legacy_runtime::run(editor_requested);
        }
